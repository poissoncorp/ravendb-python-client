...
class PullReplicationMode(enum.Enum):
    """Determines the direction of data flow between a pull-replication hub and sink.

    Mirrors the C# ``[Flags]`` enum - the combined value serializes as the
    comma-separated string the server expects/returns.
    """
...
class ReplicationNode:
    ...
    @classmethod
    def from_json(cls, json_dict: Dict[str, Any]) -> Optional[ReplicationNode]:
        if json_dict is None:
            return None
        return cls(json_dict.get("Url"), json_dict.get("Database"), json_dict.get("Disabled", False))
...
class ExternalReplication(ExternalReplicationBase):
    ...
    @classmethod
    def from_json(cls, json_dict: Dict[str, Any]) -> Optional[ExternalReplication]:
        if json_dict is None:
            return None
        obj = cls()
        obj._fill_from_json(json_dict)
        delay = json_dict.get("DelayReplicationFor")
        obj.delay_replication_for = Utils.string_to_timedelta(delay) if delay else None
        return obj
...
class ReplicationHubAccess:
    ...
    @classmethod
    def from_json(cls, json_dict: Dict[str, Any]) -> Optional[ReplicationHubAccess]:
        if json_dict is None:
            return None
        return cls(
            json_dict.get("Name"),
            json_dict.get("CertificateBase64"),
            json_dict.get("AllowedHubToSinkPaths"),
            json_dict.get("AllowedSinkToHubPaths"),
        )
