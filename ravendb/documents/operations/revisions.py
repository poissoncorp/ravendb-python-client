...
class RevisionsOperationContinuationParameters:
    ...
    @classmethod
    def from_json(cls, json_dict: Dict[str, Any]) -> RevisionsOperationContinuationParameters:
        return cls(
            json_dict.get("StartFromEtags"),
            json_dict.get("EtagBarriers"),
            json_dict.get("NodeTags"),
        )
...
class EnforceRevisionsConfigurationOperation(IOperation[OperationIdResult]):
    """Applies the current revisions configuration to all existing revisions at once.

    This is a long-running operation - send it via ``store.operations.send_async`` and
    await completion on the returned :class:`Operation`.
    """
...
class AdoptOrphanedRevisionsOperation(IOperation[OperationIdResult]):
    """Re-attaches orphaned revisions (revisions of a deleted document missing from the
    revisions bin) to their documents.

    Long-running - send via ``store.operations.send_async`` and await completion.
    """
...
class RevisionsBinConfiguration:
    ...
    @classmethod
    def from_json(cls, json_dict: Dict[str, Any]) -> RevisionsBinConfiguration:
        return cls(
            json_dict.get("Disabled", False),
            json_dict.get("MinimumEntriesAgeToKeepInMin"),
            json_dict.get("CleanerFrequencyInSec"),
        )
...
class ConfigureRevisionsBinCleanerOperation(MaintenanceOperation[ConfigureRevisionsBinCleanerOperationResult]):
    ...
    class ConfigureRevisionsBinCleanerCommand(
        RavenCommand[ConfigureRevisionsBinCleanerOperationResult], RaftCommand
    ):
        def __init__(self, configuration: RevisionsBinConfiguration):
